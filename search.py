...
def best_first_graph_search(problem, f, display=False):
    ...
    while frontier:
        ...
        if problem.goal_test(node.state):
            ...
            print("Total nodes removed is " + str(counter))
            return node
        explored.add(node.state)
        for child in node.expand(problem):
            if child.state not in explored and child not in frontier:
...
class EightPuzzle(Problem):
    ...
    def manhattanHeuristic(self, node):
        ...
        manhattanDistances.append([0,1,2,1,2,3,2,3,4])
        manhattanDistances.append([1,0,1,2,1,2,3,2,3])
        manhattanDistances.append([2,1,0,3,2,1,4,3,2])
        manhattanDistances.append([1,2,3,0,1,2,1,2,3])
        manhattanDistances.append([2,1,2,1,0,1,2,1,2])
        manhattanDistances.append([3,2,1,2,1,0,3,2,1])
        manhattanDistances.append([2,3,4,1,2,3,0,1,2])
        manhattanDistances.append([3,2,3,2,1,2,1,0,1])
        manhattanDistances.append([4,3,2,3,2,1,2,1,0])
        
        for i in range(len(state)):
            if state[i] == 0:
                continue
            else:
                distance = distance + manhattanDistances[state[i]][i]
                #print(distance)
        #print(distance)
        return distance
    ...
    def h(self, node):
        ...
        if self.manhattanH == False and self.findMaxH == False:
            #print("Using default misplaced tile heuristic...")
            #print(sum(s != g for (s, g) in zip(node.state, self.goal)))
            return sum(s != g for (s, g) in zip(node.state, self.goal))
        elif self.manhattanH == True and self.findMaxH == False:
            #print("Using manhattan heuristic...")
            return self.manhattanHeuristic(node)
        else:
            #print("Finding max value heuristic...")
            misplacedTileCalc = sum(s != g for (s, g) in zip(node.state, self.goal))
            manhattanCalc = self.manhattanHeuristic(node)
            #print(max(misplacedTileCalc, manhattanCalc))
            return max(misplacedTileCalc, manhattanCalc)
...
class DuckPuzzle(Problem):
    ...
    def actions(self, state):
        ...
        """SPECIAL UP AND SPECIAL DOWN WERE ADDED FOR THE CASE OF SPOTS 0, 1, 2, and 3 in the field. These 4 spots cannot have UP or DOWN applied to them since their spot in the list will only move by 2 because of the house shaped grid. Previously they could move 3 spots in the list when the grid was square"""
        
        possible_actions = ['UP', 'DOWN', 'LEFT', 'RIGHT', 'SPECIALUP', 'SPECIALDOWN']       
        index_blank_square = self.find_blank_square(state)

        if index_blank_square == 0:
            possible_actions.remove('LEFT')
            possible_actions.remove('UP')
            possible_actions.remove('SPECIALUP')
            possible_actions.remove('DOWN')
        ...
        if index_blank_square == 2:
            ...
            possible_actions.remove('UP')
            possible_actions.remove('DOWN')
            possible_actions.remove('SPECIALDOWN')
        if index_blank_square == 3:
            possible_actions.remove('UP')
            possible_actions.remove('SPECIALDOWN')
    ...
    def result(self, state, action):
        ...
        blank = self.find_blank_square(state)
        new_state = list(state)
        #print("The blank is at spot " + str(blank))
        #print("The list is currently " + str(new_state))

        delta = {'UP':-3, 'DOWN':3, 'LEFT':-1, 'RIGHT':1, 'SPECIALUP':-2, 'SPECIALDOWN':2}
        neighbor = blank + delta[action]

        #print("The action is " + str(delta[action]) + " and the neighbor is " + str(neighbor))
        new_state[blank], new_state[neighbor] = new_state[neighbor], new_state[blank]
    ...
    def manhattanHeuristic(self, node):
        ...
        manhattanDistances = []
        manhattanDistances.append([])
        manhattanDistances.append([0,1,1,2,3,4,3,4,5])
        manhattanDistances.append([1,0,2,1,2,3,2,3,4])
        manhattanDistances.append([1,2,0,1,2,3,2,3,4])
        manhattanDistances.append([2,1,1,0,1,2,1,2,3])
        manhattanDistances.append([3,2,2,1,0,1,2,1,2])
        manhattanDistances.append([4,3,3,2,1,0,3,2,1])
        manhattanDistances.append([3,2,2,1,2,3,0,1,2])
        manhattanDistances.append([4,3,3,2,1,2,1,0,1])
        manhattanDistances.append([5,4,4,3,2,1,2,1,0])
    ...
    """Regular Misplaced Tile Heuristic"""
    def h(self, node):
        """Return the heuristic value for a given state. Default heuristic function used is 
        h(n) = number of misplaced tiles"""
